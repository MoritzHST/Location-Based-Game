...
module.exports = {
  ...
  checkIfValidQuery: function(pQuery) {
    return (!query.hasOwnProperty(undefined) && Object.keys(pQuery).length > 0);
  },
  ...
  dbResult: function(pErr, pRes, pResult, pResultType) {
    if (pErr) {
      logging.Error(pErr);
      pRes.status(422).jsonp({"error": "Es ist ein Fehler aufgetreten beim Versuch sich mit der Datenbank zu verbinden!"});
    }
    else {
      if (!pResult)
        pRes.status(422).jsonp({"error": pResultType });
      else
